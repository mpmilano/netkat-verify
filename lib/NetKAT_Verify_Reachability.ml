...
(you gave forwards_topo this: %s)" (NetKAT_Pretty.string_of_policy pol))
	...
	let max_k pol : int = 
	  let open NetKAT_Dehop_Graph in
	      let (used_bits : int) = 
		List.fold_left 
		  (fun (a : int) x -> 
		    let used_bits : int = 
		      Sat.bitvec_size (header_to_zsort x) in
		    used_bits + a) 0 all_fields  in
	      let longest_realpath = longest_shortest (parse_graph pol) in
	      longest_realpath * used_bits
	  
	    
	let run_sat_reachability 
	    inpkt_constr pt outp_constr hist_expr str = 
	  let k = max_k pt in
	  ...
	  let history_sets = 
		...
		| _ -> (forwards_k k inpt pol topo outp)
		  ::(build_hists (k-1))in
	    build_hists k in
	  let results = parallel_map 
	    (fun x ->
	      ...
	      let prog = 
		(Sat.assemble_program 
		   ((Support_Code.declare_datatypes k) 
		    @ indecl::outdecl::decl)
		   (ZProgram [
		     hist_expr hist;
	      ...
	      let _ = Sat_Utils.printdebug_prog Sat.serialize_program prog 
		(Printf.sprintf "%s-with-%u-pkts" str (List.length hist)) in
	      let res,exec_time = 
		Sat.solve prog in
	      res,hist
	    )
	...
	let rec define_relation pol = 
	  ...
	  with Not_found -> 
	    let sym,decl = 
	      fresh (SRelation [SHistory; SHistory]) in
...
let check_reachability_z3_ints str ints inp pol outp hist_fun oko = 
  ...
  match run_sat_reachability inp pol outp hist_fun str ,oko with 
	| true,true -> true
	| false,false -> true
	| _ -> false
...
let check_waypoint str inp pol outp (wayp : VInt.t) ok = 
  ...
  then
	...
	(ZAnd 
	   (List.map 
	      (fun x -> 
		pred_test_not Switch (TVar x) 
		  (Sat_Utils.encode_vint wayp Switch)
	      ) l)) in
    (check_reachability_z3_ints str
       (Sat_Utils.collect_constants 
	  (Seq (Filter (Test (Switch, wayp)), 
		(Seq (Seq (Filter inp,pol),Filter outp)))))
